...
export default function BukuBesar() {
  ...
  const fetchTransaksi = async () => {
    ...
    if (!error) {
      ...
      setCurrentPage(1); // Reset paginasi ke halaman 1 setiap kali filter berubah
    }
    setIsLoading(false);
  };
  ...
  }, [startDate, endDate, selectedKegiatan, selectedKategori]);
  ...
  const handleExportExcel = () => {
    ...
    transaksiList.forEach((row, index) => {
      ...
      csv += `${index + 1}${separator}${row.tanggal_transaksi}${separator}${keg}${separator}${kat}${separator}${ket}${separator}${masuk}${separator}${keluar}${separator}${saldo}\n`;
    });

    csv += `\nTOTAL${separator}${separator}${separator}${separator}${separator}${totalMasuk}${separator}${totalKeluar}${separator}${saldo}\n`;
  ...
  // MENGHITUNG SALDO BERJALAN & TOTAL UNTUK SELURUH DATA
  let tempSaldo = 0;
    ...
    <DashboardLayout>
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          .flex-1, .overflow-y-auto, .overflow-x-auto, .h-screen { display: block !important; height: auto !important; overflow: visible !important; }
      ...
      <div className="print:bg-white print:text-black space-y-6">
        ...
        {/* AREA FILTER DENGAN FITUR CANCEL (RESET) */}
        <div className={`print:hidden p-5 rounded-2xl border shadow-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'}`}>
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              {startDate !== defaultStart && (
                <button onClick={() => setStartDate(defaultStart)} className="text-[10px] font-bold text-rose-500 hover:text-rose-600 flex items-center bg-rose-500/10 px-1.5 py-0.5 rounded-md transition-colors">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              ...
              {endDate !== defaultEnd && (
                <button onClick={() => setEndDate(defaultEnd)} className="text-[10px] font-bold text-rose-500 hover:text-rose-600 flex items-center bg-rose-500/10 px-1.5 py-0.5 rounded-md transition-colors">
                  <X size={10} className="mr-0.5"/> Reset
                </button>
              )}
            </div>
          ...
          <div className="relative" ref={dropdownKegRef}>
            <div className="flex justify-between items-center mb-1.5">
              ...
              {selectedKegiatan.length > 0 && (
                <button onClick={() => setSelectedKegiatan([])} className="text-[10px] font-bold text-rose-500 hover:text-rose-600 flex items-center bg-rose-500/10 px-1.5 py-0.5 rounded-md transition-colors">
                  <X size={10} className="mr-0.5"/> Reset
                </button>
              )}
            </div>
            <button onClick={() => setIsDropdownKegOpen(!isDropdownKegOpen)} className={`w-full flex justify-between items-center p-2.5 border rounded-xl outline-none font-medium text-sm text-left transition-colors ${isDarkMode ? 'bg-[#1e293b] border-slate-700 text-slate-200' : 'bg-gray-50 border-gray-300 text-gray-900'}`}>
              <span className="truncate">{selectedKegiatan.length === 0 ? "Semua Kegiatan" : `${selectedKegiatan.length} Dipilih`}</span>
              <ChevronDown size={16} className={`transition-transform ${isDropdownKegOpen ? 'rotate-180' : ''}`} />
            </button>
            {isDropdownKegOpen && (
          ...
          <div className="relative" ref={dropdownKatRef}>
            <div className="flex justify-between items-center mb-1.5">
              ...
              {selectedKategori.length > 0 && (
                <button onClick={() => setSelectedKategori([])} className="text-[10px] font-bold text-rose-500 hover:text-rose-600 flex items-center bg-rose-500/10 px-1.5 py-0.5 rounded-md transition-colors">
                  <X size={10} className="mr-0.5"/> Reset
                </button>
              )}
            </div>
            <button onClick={() => setIsDropdownKatOpen(!isDropdownKatOpen)} className={`w-full flex justify-between items-center p-2.5 border rounded-xl outline-none font-medium text-sm text-left transition-colors ${isDarkMode ? 'bg-[#1e293b] border-slate-700 text-slate-200' : 'bg-gray-50 border-gray-300 text-gray-900'}`}>
              <span className="truncate">{selectedKategori.length === 0 ? "Semua Kategori" : `${selectedKategori.length} Dipilih`}</span>
              <ChevronDown size={16} className={`transition-transform ${isDropdownKatOpen ? 'rotate-180' : ''}`} />
            </button>
            {isDropdownKatOpen && (
              <div className={`absolute z-10 w-full mt-2 border rounded-xl shadow-lg max-h-60 overflow-y-auto ${isDarkMode ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-gray-200'}`}>
                <div className={`flex items-center p-3 cursor-pointer border-b ${isDarkMode ? 'hover:bg-slate-800 border-slate-700' : 'hover:bg-gray-50 border-gray-100'}`} onClick={() => setSelectedKategori([])}>
                  {selectedKategori.length === 0 ? <CheckSquare size={18} className={isDarkMode ? 'text-cyan-400 mr-2' : 'text-blue-600 mr-2'} /> : <Square size={18} className="text-gray-400 mr-2" />}
                  <span className="font-bold text-sm">Semua</span>
                </div>
                {kategoriList.map(k => (
                  <div key={k.id_kategori} className={`flex items-center p-3 cursor-pointer ${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-gray-50'}`} onClick={() => toggleKategori(k.id_kategori)}>
                    {selectedKategori.includes(k.id_kategori) ? <CheckSquare size={18} className={isDarkMode ? 'text-cyan-400 mr-2' : 'text-blue-600 mr-2'} /> : <Square size={18} className="text-gray-400 mr-2" />}
                    <span className="text-sm font-medium">{k.nama_kategori}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className={`rounded-2xl border shadow-sm transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'} print:border-none print:shadow-none print:bg-transparent`}>
          ...
          <div className="hidden print:block text-center mb-6 pt-4 border-b-2 border-black pb-4">
            ...
            <p className="mt-1 font-medium text-black text-sm">Dicetak pada: {new Date().toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })}</p>
          </div>
          
          {/* TABEL KHUSUS LAYAR (Dengan Paginasi) */}
          ...
          {/* TABEL KHUSUS CETAK PDF (Tersembunyi di layar, muncul semua data saat print) */}
          <div className="hidden print:block w-full">
            <table className="w-full text-left whitespace-normal border-collapse border border-black text-black">
              <thead>
                <tr className="text-xs uppercase tracking-widest border-b border-black">
                  <th className="py-2 px-2 border border-black text-center">No</th>
                  <th className="py-2 px-2 border border-black">Tanggal</th>
                  <th className="py-2 px-2 border border-black">Uraian Transaksi</th>
                  <th className="py-2 px-2 border border-black text-right">Masuk (Debit)</th>
                  <th className="py-2 px-2 border border-black text-right">Keluar (Kredit)</th>
                  <th className="py-2 px-2 border border-black text-right">Saldo</th>
                </tr>
              </thead>
                ...
                {fullData.map((item) => {
                  ...
                  return (
                    <tr key={`print-${item.id_transaksi}`}>
                      <td className="py-2 px-2 border border-black text-center text-xs">{item.urutan}</td>
                      <td className="py-2 px-2 border border-black text-xs">{item.tanggal_transaksi}</td>
                      <td className="py-2 px-2 border border-black">
                        ...
                        <span className="inline-block mt-1 text-[9px] font-bold uppercase tracking-wider">{item.kegiatan?.nama_kegiatan}</span>
                      </td>
                      <td className="py-2 px-2 border border-black text-right text-sm">
                        {isPemasukan ? formatRupiah(item.jumlah) : '-'}
                      </td>
                      <td className="py-2 px-2 border border-black text-right text-sm">
                        {!isPemasukan ? formatRupiah(item.jumlah) : '-'}
                      </td>
                      <td className="py-2 px-2 border border-black text-right text-sm font-black">
                        {formatRupiah(item.computedSaldo)}
                      </td>
                    </tr>
                  );
              ...
              {fullData.length > 0 && (
                <tfoot>
                  <tr className="font-bold border-t-2 border-black">
                    <td colSpan={3} className="py-3 px-2 border border-black text-right text-sm">TOTAL MUTASI:</td>
                    <td className="py-3 px-2 border border-black text-right text-sm">{formatRupiah(totalMasukSemua)}</td>
                    <td className="py-3 px-2 border border-black text-right text-sm">{formatRupiah(totalKeluarSemua)}</td>
          ...
          {/* KONTROL PAGINASI & TOTAL (Hanya tampil di layar) */}
          <div className={`p-5 border-t print:hidden flex flex-col md:flex-row justify-between items-center gap-4 ${isDarkMode ? 'border-slate-800 bg-[#0f172a]' : 'border-gray-100 bg-gray-50'}`}>
