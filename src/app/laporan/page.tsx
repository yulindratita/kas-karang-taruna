...
import { Printer, Filter, CalendarDays, FileSpreadsheet, ChevronDown, CheckSquare, Square, Edit, Trash2, X } from 'lucide-react';
...
export default function BukuBesar() {
  ...
  // Pantau tema untuk sinkronisasi warna
  useEffect(() => {
  ...
  const handleHapus = async (id: string) => {
    const isConfirm = window.confirm('Yakin ingin menghapus secara permanen transaksi ini?');
    if (!isConfirm) return;
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left whitespace-nowrap print:border-collapse print:border print:border-black print:text-black">
              ...
              <tbody className={`divide-y print:divide-black ${isDarkMode ? 'divide-slate-800/80' : 'divide-gray-100'}`}>
                {isLoading ? (
                  <tr><td colSpan={isBisaEdit ? 7 : 6} className="py-12 text-center text-sm opacity-50">Memuat data...</td></tr>
                ) : transaksiList.length === 0 ? (
                  <tr><td colSpan={isBisaEdit ? 7 : 6} className="py-12 text-center text-sm opacity-50">Tidak ada transaksi.</td></tr>
                ) : (
