...
import { Trash2, AlertTriangle, ArrowRight, TrendingUp, ArrowDownToLine, ArrowUpFromLine, Wallet } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchDataDashboard = async () => {
    ...
    // 1. Ambil seluruh transaksi untuk menghitung total statistik
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Ringkasan kondisi finansial organisasi secara real-time.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={`p-6 rounded-2xl border shadow-sm relative overflow-hidden transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`p-3 rounded-xl ${isDarkMode ? 'bg-cyan-500/20 text-cyan-400' : 'bg-blue-50 text-blue-600'}`}>
                <Wallet size={24} />
              </div>
            </div>
            <p className={`text-[11px] font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Akumulasi seluruh kas masuk dan keluar</p>
          </div>
          ...
          <div className={`p-6 rounded-2xl border shadow-sm relative overflow-hidden transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`p-3 rounded-xl ${isDarkMode ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}>
                <ArrowDownToLine size={24} />
              </div>
            </div>
            <p className={`text-[11px] font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Jumlah total dana masuk (Debit)</p>
          </div>
          ...
          <div className={`p-6 rounded-2xl border shadow-sm relative overflow-hidden transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`p-3 rounded-xl ${isDarkMode ? 'bg-rose-500/20 text-rose-400' : 'bg-rose-50 text-rose-600'}`}>
                <ArrowUpFromLine size={24} />
              </div>
            </div>
            <p className={`text-[11px] font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Jumlah total dana keluar (Kredit)</p>
          </div>
        ...
        <div className={`rounded-2xl border shadow-sm overflow-hidden transition-colors ${isDarkMode ? 'bg-[#0f172a] border-slate-800/80' : 'bg-white border-gray-200'}`}>
          <div className={`p-5 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
            ...
            <Link href="/laporan" className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors flex items-center ${isDarkMode ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left whitespace-nowrap">
              ...
              <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800/80' : 'divide-gray-100'}`}>
                {isLoading ? (<tr><td colSpan={5} className="py-12 text-center text-sm opacity-50">Memuat riwayat...</td></tr>) 
                : transaksi.length === 0 ? (<tr><td colSpan={5} className="py-12 text-center text-sm opacity-50">Belum ada transaksi tercatat.</td></tr>) 
                : transaksi.map((item) => {
